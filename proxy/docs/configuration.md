...
| `HOOKS_FILE` | string | `.claudio/hooks.json` | Workspace-relative file declaring commands to run after a successful destructive action: `{"write": ["npx prettier --write $CLAUDIO_PATH"]}`. Hooks run **without asking**, so they are inert until a person trusts the file with `npm run hooks -- trust <workspace>`, and any change to it revokes that. `$CLAUDIO_ACTION`, `$CLAUDIO_PATH` and `$CLAUDIO_FILE` are in the environment. Empty disables hooks. |
| `HOOKS_TRUST_FILE` | path | `proxy/hooks-trust.json` | Where trusted hook files are remembered, keyed by workspace and by content hash. **Outside any workspace on purpose**: inside one, the model could write it with `write` and trust its own hooks. |
