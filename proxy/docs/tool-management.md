...
> **Persistent cache:** The probe result is stored in `proxy/model-cache.json`, keyed by model ID. On subsequent proxy restarts with the same model loaded, the cached value is used immediately — no binary search runs. This avoids a 3–30s delay on every restart.
>
> To force a new detection: delete `proxy/model-cache.json` or remove the specific model entry from the file, then restart the proxy.
...
- [Architecture](architecture.md) — hexagonal structure and request flow
- [Configuration](configuration.md) — all scoring weights and tool variables
