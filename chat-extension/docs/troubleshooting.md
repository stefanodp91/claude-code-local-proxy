...
**Multi-window note:** each VS Code window with a different `proxyDir` manages its own
independent proxy process. Opening a second project no longer disrupts the first window's
connection.
